...
public final class AbstractState implements java.io.Serializable {
  ...
  public AbstractState(Object obj, Class<?> type, boolean init) {
    FDUtils.checkNull(type, "The class type could not be null.");
    ...
    if(!FDUtils.isPrimitive(type) && !type.isPrimitive()) {
        FDUtils.checkTrue(type.isAssignableFrom(this.clz), "Type: " + type.getName() + 
        		" is not an assignable form of: " + this.clz.getName());
    }
    
    //for non-null object
    if(this.clz.isPrimitive() || FDUtils.isPrimitive(this.clz)
  ...
  public static AbstractState deserialize(ObjectInputStream ois) throws IOException, ClassNotFoundException {
    ...
    FDUtils.checkTrue(obj instanceof AbstractState, "Object obj: " + obj + " is not the type: AbstractState");
  ...
  private void saveFieldStates() {
    for(int i = 0; i < this.fields.length; i++) {
      ...
      FDUtils.checkNull(this.serializableFields[i], "The " + i + "-th field in the serializableField array" +
      		" could not be null.");
    }
  }
  ...
  private void recoverFieldStates() {
    FDUtils.checkNull(this.clz, "The clz field could not be null.");
    ...
    for(int i = 0; i < this.serializableFields.length; i++) {
      ...
      FDUtils.checkNull(field, "The field could not be null.");
      this.fields[i] = field;
    }
  }
  ...
  public String toString() {
    ...
    FDUtils.checkTrue(this.fields.length == this.states.length, "The length of field: " + fields.length
    		+ " != the length of states: " + states.length );
    ...
    if(this.isPrimtiveOrString) {
      FDUtils.checkTrue(this.valueOfPrimitiveOrString != null, "The valueOfPrimitiveOrString could not be null.");
