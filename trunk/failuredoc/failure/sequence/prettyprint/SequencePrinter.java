...
class SequencePrinter {
	...
	public SequencePrinter(ExecutableSequence sequenceToPrint, VariableRenamer renamer) {
		this.sequenceToPrint = sequenceToPrint;
		this.renamer = renamer;
	}
	
	/**
	 * Dump a sequence statement by statement, with variables renamed.
	 * */
	public String printToString() {
		...
		for(int i = 0; i < this.sequenceToPrint.sequence.size(); i++) {
			...
			//print statement one by one
			StringBuilder oneStatement = new StringBuilder();
			
			//append the statement code
		    ...
		    List<Check> exObs = this.sequenceToPrint.getChecks(i, ExpectedExceptionChecker.class);
		    if (!exObs.isEmpty()) {
		      FDUtils.checkTrue(exObs.size() == 1, "The size of exceptional checker should be 1, now it is: "
		    		  + exObs.size());
		      Check o = exObs.get(0);
...
//	      sb.append(Reflection.getCompilableName(rmethod.getMethod().getReturnType()));
	      sb.append(Reflection.getCompilableSimpleName(rmethod.getMethod().getReturnType()));
	      String cast = "";
...
//	    String declaringClassStr = Reflection.getCompilableName(declaringClass);
	    String declaringClassStr = Reflection.getCompilableSimpleName(declaringClass);
	...
	private void printPrimitiveType(PrimitiveOrStringOrNullDecl statement,
	    ...
	    if (type.isPrimitive()) {
	      ...
	      sb.append(Reflection.getCompilableSimpleName(type));
	      sb.append(" ");
	      sb.append(this.renamer.getRenamedVar(newVar.index));
	      sb.append(" = ");
	      sb.append(PrimitiveTypes.toCodeString(statement.getValue()));
	...
	public void appendReceiverOrClassForStatics(RMethod rmethod, String receiverString,
			StringBuilder b) {
		if (rmethod.isStatic()) {
