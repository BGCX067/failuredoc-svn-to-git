       ...
       if (GenInputsAbstract.forbid_null && GenInputsAbstract.check_npe) {
         ...
         if (exec.getException().getClass().equals(NullPointerException.class)) {
    	   ...
    	   if (GenInputsAbstract.forbidden_exception != null) {
               ExceptionalExecution exec = (ExceptionalExecution)s.getResult(idx);
               if (exec.getException().getClass().getName().equals(GenInputsAbstract.forbidden_exception)) {
    			try {
    				Class<? extends Throwable> exception_clz = (Class<? extends Throwable>) Class.forName(GenInputsAbstract.forbidden_exception);
    	             ForbiddenExceptionChecker obs = new ForbiddenExceptionChecker(exception_clz);
    	             s.addCheck(idx, obs, false);
