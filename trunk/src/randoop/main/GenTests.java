...
import cov.Branch;
import cov.Coverage;
import experiment.ddisolate.FaultyStatementIsolator;
...
import failuredoc.analysis.ddmin.StatementMinimizer;
import failuredoc.analysis.simplify.SequenceUnusedVarSimplifier;
import failuredoc.analysis.simplify.TypeBasedSimplifier;
...
//     System.setSecurityManager(randoopSecurityManager);
    ...
    if(GenInputsAbstract.ddmin) {
    	GenInputsAbstract.long_format = true;
    	//try to simplifying the test
    	FDUtils.checkTrue(explorer instanceof ForwardGenerator, "should be an forward generator");
    	MultiVisitor visitor = explorer.executionVisitor;
        //get all failure sequences
        List<ExecutableSequence> failed_sequences = FDUtils.failureSequences(sequences);
        if(!failed_sequences.isEmpty()) {
        	List<ExecutableSequence> minimized_sequences = new LinkedList<ExecutableSequence>();
        	//do the simplification work one by one
        	FDUtils.stdln("Simplifying failed sequences via running delta debugging minimizer.");
        	for(ExecutableSequence fseq : failed_sequences) {
        		List<Integer> list = new LinkedList<Integer>();
        		for(int i = 0; i < fseq.sequence.size(); i++) {
        			list.add(i);
        		}
        		StatementMinimizer minimizer = new StatementMinimizer(list, fseq, visitor);
        		ExecutableSequence minimized_sequence = minimizer.minimize_sequence();
        		minimized_sequences.add(minimized_sequence);
        		FDUtils.stdln("Sequence size before minimization: " + fseq.sequence.size());
        		FDUtils.stdln("  Sequence size after minimization: " + minimized_sequence.sequence.size());
        	}
        	String backup = junit_classname;
        	junit_classname = junit_classname + "_simplified_ddmin";
        	write_junit_tests("./failuredoc_output", minimized_sequences);
        	junit_classname = backup;
        	FDUtils.stdln("Dumped simplified failed sequences via ddmin.");
        }
    }

    if(GenInputsAbstract.simplifying) {
    	GenInputsAbstract.long_format = true;
    	//try to simplifying the test
    	FDUtils.checkTrue(explorer instanceof ForwardGenerator, "should be an forward generator");
    	MultiVisitor visitor = explorer.executionVisitor;
        ...
        List<ExecutableSequence> failed_sequences = FDUtils.failureSequences(sequences);
        if(!failed_sequences.isEmpty()) {
        	List<ExecutableSequence> simplified_sequences = new LinkedList<ExecutableSequence>();
        	//do the simplification work one by one
        	FDUtils.stdln("Simplifying failed sequences via removing unused vars.");
        	for(ExecutableSequence fseq : failed_sequences) {
        		SequenceUnusedVarSimplifier simplifier = new SequenceUnusedVarSimplifier(fseq.sequence, visitor);
        		ExecutableSequence simplified_sequence = simplifier.simplfy_sequence();
        		FDUtils.stdln("Number of statement removed via removing unused vars: " + simplifier.removed_indices.size()
        				+ ", length of original sequence: " + simplifier.eSeq.sequence.size());
        		simplified_sequences.add(simplified_sequence);
        	}
        	//write to the dir
        	String backup = junit_classname;
        	junit_classname = junit_classname + "_simplified";
        	write_junit_tests("./failuredoc_output", simplified_sequences);
        	junit_classname = backup;
        	FDUtils.stdln("Dumped simplified failed sequences via removing unused vars.");
        	
        	//doing further simplification using type based information
        	if(GenInputsAbstract.typebased_simplified) {
        		FDUtils.stdln("Simplifying failed sequences via type based info.");
        		List<ExecutableSequence> simplified_sequence_after_type = new LinkedList<ExecutableSequence>();
        		for(ExecutableSequence s_seq : simplified_sequences) {
        			TypeBasedSimplifier simplifer = new TypeBasedSimplifier(s_seq.sequence, visitor);
        			ExecutableSequence simplified_sequence = simplifer.simplfy_sequence();
        			FDUtils.stdln("Number of statement removed via used type based information: " + simplifer.removed_indices.size()
        					+ ", length of original sequence: " + simplifer.eSeq.sequence.size());
        			simplified_sequence_after_type.add(simplified_sequence);
        		}
        		//write to the dir
            	backup = junit_classname;
            	junit_classname = junit_classname + "_simplified_simplified";
            	write_junit_tests("./failuredoc_output", simplified_sequence_after_type);
            	junit_classname = backup;
            	FDUtils.stdln("Dumped simplified sequences via type based info.");
        	}
        }
    }
    
    //do the failure doc analysis
    if(GenInputsAbstract.failure_doc) {
      //set the long-format to be true by force
      GenInputsAbstract.long_format = true;
      FDUtils.checkTrue(explorer instanceof ForwardGenerator, "should be an forward generator");
      SequenceCollection components = ((ForwardGenerator)explorer).components;
      //get all failure sequences
      List<ExecutableSequence> failed_sequences = FDUtils.failureSequences(sequences);
      //failed_sequences.remove(1);
      //start to do failure analysis
      if(!failed_sequences.isEmpty()) {
    	  
    	  if(failure.main.Main.failed_seq_output != null) {
    		  System.out.println("Writing " + failed_sequences.size() + " failed sequences to: "
    				  + failure.main.Main.failed_seq_output);
    		  SequenceFactory.writeSequenceToFile(failed_sequences, failure.main.Main.failed_seq_output);
//    		  for(ExecutableSequence eseq : failed_sequences) {
//    			  FaultyStatementIsolator isolator = new FaultyStatementIsolator(eseq.sequence, VisitorFactory.createDefaultVisitor());
...
//    		  System.exit(1);
          ...
          FailureTestsAnalyzer analyzer =
    	      new FailureTestsAnalyzer(failed_sequences, components, explorer.executionVisitor);
...
//      System.out.println("Failed test size: " + failed_sequences.size());
//      write_junit_tests("./failuredoc_output", failed_sequences);
    }
